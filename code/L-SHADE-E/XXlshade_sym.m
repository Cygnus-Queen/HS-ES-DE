...
%max_nfes_for_size = problem_size * beishu;
...
for func = [9:20]
    ...
    for run_id = 1 : 30
        ...
        while nfes < max_nfes
            ...
            if sign == 1 && pop_size == our_mem_pop_size
                curdiv = 0.0;
                for x =1 : problem_size
                    midpoint(x) = median(pop(:,x));
                end
...
%                 p1 = pop(r3,:);
%                 p2 = pop(r4,:);                
%                 vi_ = pop + sf(1:j, ones(1, problem_size)) .* (p1(1:j,:) - p2(1:j,:)); 
%                 vi = vi_;
%                 %[x1,x2]=size(pop);
%                 %fprintf('%d %d ',pop_size,x1);
%                 %disp(pop_size);
%                 %disp('sizeof(pop)');
%                 %pop_size = our_mem_pop_size;
            ...
            mask = rand(j, problem_size) > cr(1:j, ones(1, problem_size)); % mask is used to indicate which elements of ui comes from the parent
            rows = (1 : j)'; cols = floor(rand(j, 1) * problem_size)+1; % choose one position where the element of ui doesn't come from the parent
            jrand = sub2ind([j problem_size], rows, cols); mask(jrand) = false;
            ui = vi; 
            ui(mask) = pop(mask);               
            end
...
%                 end
            ...
            if sign == 0
                ...
                if renfes~= nfes
                     plan_pop_size = round((((min_pop_size - our_mem_pop_size) / max_nfes) * renfes) + our_mem_pop_size);
                else  
...
% % % % % % % % % %                     fjdkfjkdjfkdjkfjdkfj
% % % % % % % % % %                     if 2 * pop_size <= our_mem_pop_size
...
% % % % % % % % % %                         
% % % % % % % % % %                                        
% % % % % % % % % %                         temp = [children_fitness; fitness];
% % % % % % % % % %                         [mt, nt] = size(temp);
% % % % % % % % % %                         temp2 = [pop; ui];
% % % % % % % % % %                         [M, N] = size(pop);
% % % % % % % % % %                         fitness_ = zeros(mt,nt);
% % % % % % % % % %                         fitness_ = temp(1:our_mem_pop_size,1:1);
% % % % % % % % % %                         fitness = fitness_;
% % % % % % % % % %                         popold_ = temp2(1:our_mem_pop_size, 1:N );%%%%%%%
% % % % % % % % % %                         popold = popold_;
% % % % % % % % % %                         pop = popold;
% % % % % % % % % %                         disp(pop_)
% % % % % % % % % %                         pop_size = our_mem_pop_size;                       
% % % % % % % % % %                     end
            ...
            if (nfes / max_nfes) >= (xxx / 100)
                if  xxx > 0
                    ...
                    curdiv = curdiv / pop_size;
                    %disp([num2str(nfes),'  ',num2str(min(fitness)-optimum),'  ',num2str(curdiv),'  ',num2str(everbestfitall-optimum)])
                    fprintf(fp,'%d %e %e %e %e\r\n', nfes, curdiv, mean(fitness)-optimum, min(fitness)-optimum, everbestfitall);
                end
                xxx = xxx + 1;
            end
        ...
        bsf_error_val = everbestfitall;%bsf_fit_var - optimum;这句被我改了
