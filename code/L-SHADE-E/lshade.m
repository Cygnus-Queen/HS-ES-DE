...
fhd=@cec14_func;
for func = [1:30]
    filename = strcat(strcat('F',num2str(func)),'_originaltt.txt');
    fp = fopen(filename,'a+');
   filename2 = strcat(strcat('results','_.txt'));
           ...
           if everbestfitall > min(fitness)-optimum
            ...
            if (nfes / max_nfes) >= (xxx / 100)
                if  xxx > 0
                    ...
                    curdiv = curdiv / pop_size;                    
        fprintf(fp,'%d %e %e %e %e %d\r\n', nfes, curdiv, mean(fitness)-optimum, min(fitness)-optimum, everbestfitall,pop_size);   
                end
                xxx = xxx + 1;
            end
        end
