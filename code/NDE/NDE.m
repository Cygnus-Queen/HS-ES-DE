...
for funcindex=[1:30]
    ...
    filename = strcat(strcat('f',num2str(funcindex)),'_original.txt');   
    fp = fopen(filename,'a+');       
      filename2 = strcat('result','_original.txt');   
        ...
        while FES<=FES_max
         ...
         %   F=randcauchy(1,NP,Fm,0.1);
          ...
          %% Execute the neighborhood-based adaptive evolution mechanism
            ...
            for i=1:NP
                if Numg(i)==gm
                    if rand > Nums(i)/gm
                        ...
                        t = rand(1, D) <1-min((FES/FES_max),(max(Tempfitness)-Tempfitness(i))/(max(Tempfitness)-min(Tempfitness)));
                        t_ = 1 - t;
                        if var(i)<mean(var)
            ...
            NPnew=round(NP_ini+(NP_end-NP_ini)*FES/FES_max);
            if NPnew<NP
                Divnum=NP-NPnew;
