...
for problemIndex = [1:9]
    
    problem = problemIndex; %problemSet(problemIndex);
    filename = strcat(strcat('f',num2str(problem)),'_10_restart.txt');    
    fp = fopen(filename,'a+');    
    Solve = [];
...
%             [valBest indBest] = sort(valParents2, 'ascend');
           ...
           if sign == 1
               ...
               mixPop = repmat(lu(1, :), mixPopSize, 1) + rand(mixPopSize, D) .* (repmat(lu(2, :) - lu(1, :), mixPopSize, 1));
               mixVal = cec14_func(mixPop',problem)'-100*problem;
               pop1 = mixPop(arrayFirst,:);
               pop2 = mixPop(arraySecond,:);
               pop3 = mixPop(arrayThird,:);
               prebestfit =realmax();
               curbestfit=realmax();
           end
           ...
           if sign == 0 && rem(FES,n * 100 * times ) == 0
               curdiv = 0.0;
               for x =1 : n
                   midpoint(x) = median(mixPop(:,x));
               end
