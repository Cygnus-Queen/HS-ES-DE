...
for problemIndex = [27:30,1:26]
    
    problem = problemIndex; %problemSet(problemIndex);
    filename = strcat(strcat('f',num2str(problem)),'_1f1_09.txt');
    fp = fopen(filename,'a+');
    Solve = [];
    ...
    while time <= totalTime
        ...
        mixVal = cec17_func(mixPop',problem)'-100*problem;
        ...
        while  FES < MaxFES
            ...
            if sign == 0
                ...
                if ~isempty(arrayFirst)
                    ...
                    [r3, r4] = gnR1R2(popsize, size(popAll, 1), r0);
                    ...
                    valOffspring1 = cec17_func(ui',problem)'-100*problem;
                    FESj = FESj + popsize;
                    FES = FES + popsize;
                ...
                if ~isempty(arraySecond)
                    ...
                    ui = vi;
                    valOffspring2 = cec17_func(ui',problem)'-100*problem;
                ...
                if ~isempty(arrayThird)
                    ...
                    valOffspring3 = cec17_func(ui',problem)'-100*problem;
            ...
            if everbestfitall > min(mixVal)
                ...
                if ~isempty(arrayFirst)
                    ...
                    pbest = pop1(indBest(randindex), :); % randomly choose one of the top 100p% solutions
                    % == == == == == == == == == == == == == == == Mutation == == == == == == == == == == == == ==
                    vi = pop1 + (pop1(r1, :) - pop1(r3, :));

                    if sum(problem ==[7,25]) == 0
                        vi = boundConstraint(vi, pop1, lu);
                    end
                    % == == == == = Crossover == == == == =
                    mask = rand(popsize, n) < 0.5; % mask is used to indicate which elements of ui comes from the parent
                    ui = vi; ui(mask) = pop1(mask);
                    
                    if curdiv > thediv || tttt == 1000
                        ui_sym = repmat(lu(2, :) + lu(1, :), popsize, 1) - ui; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %size(ui)
                        S = rand(popsize,1) < migrate;
                        ui(S,:) = ui_sym(S,:);
                    end
                    ...
                    if curdiv > thediv || tttt == 1000
                        ...
                        %disp(size(valOffspring1));
                        [mmm, nnn] = size(cec17_func(ui',problem));
                        valOffspring1 = ones(mmm, nnn) * realmax();
                    end
                    if curdiv > thediv || tttt == 1000
                    ...
                    % == == == == == == == == == == == == == == == Selection == == == == == == == == == == == == ==
                    % I == 1: the parent is better; I == 2: the offspring is better
                        valParents1 = valOffspring1;
                        popold1 = pop1;
                        popold1 = ui;
                        goodCR = CRj;
                        goodF = Fj;
                        mixPop(arrayFirst,:) = popold1;
                        mixVal(arrayFirst) = problem *100 * 10000;
                ...
                if ~isempty(arraySecond)
                    ...
                    % == == == == = Crossover == == == == =
                    mask = rand(popsize2, n) < 0.5; % mask is used to indicate which elements of ui comes from the parent
                    ui = vi; ui(mask) = pop2(mask);                    
                    
                    if curdiv > thediv || tttt == 1000
                        ui_sym = repmat(lu(2, :) + lu(1, :), popsize2, 1) - ui; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %size(ui)
                        S = rand(popsize2,1) < migrate;
                        ui(S,:) = ui_sym(S,:);
                    end
                    if curdiv > thediv || tttt == 1000
                        valOffspring2 = cec17_func(ui',problem)'-100*problem;
                        ...
                        %disp(size(valOffspring1));
                        [mmm, nnn] = size(cec17_func(ui',problem));
                        valOffspring2 = ones(mmm, nnn) * realmax();
                    end
                    %valOffspring2 = benchmark_func(ui, problem, o, A, M, a, alpha, b);
                    if curdiv > thediv || tttt == 1000
                        FES = FES + popsize2;
                    ...
                    % == == == == == == == == == == == == == == == Selection == == == == == == == == == == == == ==
                        % I == 1: the parent is better; I == 2: the offspring is better
                        valParents2 = valOffspring2;
                        popold2 = pop2;
                        popold2 = ui;
                        goodCR2 = CR2;
                        goodF2 = F2;
                        mixPop(arraySecond,:) = popold2;
                        mixVal(arraySecond) = problem * realmax();
                end
                ...
                if ~isempty(arrayThird)
                    ...
                    pm2 = pop3(a2,:);             % shuffled population 2
                    pm3 = pop3(a3,:);             % shuffled population 3
                    vi =pm1 + (pm2 - pm3); %repmat(rand(popsize3,1),1,D) .* (pm1 - pop3)
                    if sum(problem==[7,25]) ==0
                        vi = boundConstraint(vi, pop3, lu);
                    end
                    
                    mask = rand(popsize3, n) < 0.5; % mask is used to indicate which elements of ui comes from the parent
                    ui = vi; ui(mask) = pop3(mask);
                    if curdiv > thediv || tttt == 1000
                        ui_sym = repmat(lu(2, :) + lu(1, :), popsize3, 1) - ui; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %size(ui)
                        S = rand(popsize3,1) < migrate;
                        ui(S,:) = ui_sym(S,:);
                    end
                    if curdiv > thediv || tttt == 1000
                        valOffspring3 = cec17_func(ui',problem)'-100*problem;
                        ...
                        %disp(size(valOffspring1));
                        [mmm, nnn] = size(cec17_func(ui',problem));
                        valOffspring3 = ones(mmm, nnn) * realmax();
                    end
                    %valOffspring3 = benchmark_func(ui, problem, o, A, M, a, alpha, b);
                    ...
                    % == == == == == == == == == == == == == == == Selection == == == == == == == == == == == == ==
                        ...
                        goodF3 = F3;
                        mixPop(arrayThird,:) = popold3;
                        mixVal(arrayThird) = problem *100 * 10000;
                    
                end                
            end

            if sign == 0 && (FES/MaxFES) >= (xxx / 100) %rem(FES,n * 100 * times ) == 0
                curdiv = 0.0;
                for x =1 : n
                    midpoint(x) = median(mixPop(:,x));
                end
